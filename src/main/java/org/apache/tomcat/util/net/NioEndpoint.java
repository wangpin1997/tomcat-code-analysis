...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel,SocketChannel> {
    ...
    /**
     * Poller class.
     * 处理套接字线程，每个Poller内部都有一个独有的队列，Poller线程则从自己的队列中获取具体的事件任务
     * 然后将其交给Worker进行处理，Poller线程处理个数和CPU核心数有关
     */
    public class Poller implements Runnable {

        private Selector selector;
        //真正的时间队列，这个地方在之前的版本才有的是ConcurrentLinkedQueue
        //默认size为128
     ...
     * Worker是实际处理请求的线程，Worker是组件名称，真正做事情的就是SocketProcessor
     * 它是Poller 线程从自己的队列中获取任务后的真正执行者
     */
